...
def get_retriever_fn(retriever: str, **kwargs: Any) -> RetrieverFn:
    if retriever == "first_k":
        return lambda _query, _corpus, profile, num_retrieve: profile[:num_retrieve]

    elif retriever == "random":
        return lambda _query, _corpus, profile, num_retrieve: random.sample(
            profile,
            min(num_retrieve, len(profile))
        )

    elif retriever == "bm25":
        return lambda query, corpus, profile, num_retrieve: BM25Okapi(
        ...
        return RankGPT("meta-llama/Meta-Llama-3-8B-Instruct", backend="hf", **kwargs)

    elif retriever == "rank_gpt-gpt5":
        return RankGPT("gpt-5-nano", backend="openai", **kwargs)
